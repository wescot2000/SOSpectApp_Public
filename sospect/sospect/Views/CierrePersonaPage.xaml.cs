...
namespace sospect.Views
{
    ...
    public partial class CierrePersonaPage : ContentPage
    {
        private CierrePersonaViewModel _viewModel;
        private bool _isSelectorOpen = false;

        public CierrePersonaPage(AlarmaCercana alarmaCercana)
        {
            InitializeComponent();
            _viewModel = new CierrePersonaViewModel(alarmaCercana);
        ...
        private async void OnCameraButtonTapped(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result == ThreeOptionResult.Option1)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("CierrePersonaPage: Abriendo CameraPopup para foto");

                    var cameraPopup = new CameraPopup();

                    cameraPopup.PhotoCaptured += async (s, photoPath) =>
                    {
                        System.Diagnostics.Debug.WriteLine($"CierrePersonaPage: Foto capturada: {photoPath}");
                ...
                else if (result == ThreeOptionResult.Option2)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("CierrePersonaPage: iOS - usando MediaPicker.CaptureVideoAsync para video");
                    ...
                    if (videoResult != null)
                    {
                        ...
                        await ModernAlerts.ShowProcessingAsync(processingText, async () =>
                        {
                            ...
                            var videoPath = System.IO.Path.Combine(cacheDir, $"{Guid.NewGuid()}.mp4");
                            using (var source = await videoResult.OpenReadAsync())
                            using (var dest = File.Create(videoPath))
                            {
                                await source.CopyToAsync(dest);
                            }
                            System.Diagnostics.Debug.WriteLine($"CierrePersonaPage: Video iOS copiado a caché: {videoPath}");
                    ...
                    System.Diagnostics.Debug.WriteLine("CierrePersonaPage: Abriendo VideoCameraPopup para video");

                    var videoCameraPopup = new VideoCameraPopup();

                    videoCameraPopup.VideoCaptured += async (s, videoPath) =>
                    {
                        System.Diagnostics.Debug.WriteLine($"CierrePersonaPage: Video capturado: {videoPath}");
                ...
                else if (result == ThreeOptionResult.Option3)
                {
                    ...
                    if (photo != null)
                    {
                        ...
                        await ModernAlerts.ShowProcessingAsync(processingText, async () =>
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine($"CierrePersonaPage: Foto galería copiada a caché: {cachedPath}");
            ...
            {
                CrashlyticsHelper.LogError(ex, "CierrePersonaPage", "OnCameraButtonTapped");
                System.Diagnostics.Debug.WriteLine($"CierrePersonaPage OnCameraButtonTapped ERROR: {ex.Message}");
            }
            finally
            {
