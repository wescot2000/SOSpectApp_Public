...
{
    public class CierreBasicoViewModel : BaseCierreAlarmaViewModel
    {
        public ICommand ConfirmarCierreCommand { get; }

        public CierreBasicoViewModel(AlarmaCercana alarmaCercana) : base(alarmaCercana)
        {
        ...
        private async Task ConfirmarCierre()
        {
            ...
            try
            {
                ...
                {
                    var mensajeError = !string.IsNullOrEmpty(response.Message)
                        ? response.Message
                        : LblCondicionesCierreAlarma;
                    await ModernAlerts.ShowInfo(LabelInformacion, mensajeError);
                }
            }
            catch (Exception ex)
            {
                await ModernAlerts.ShowError(LabelInformacion, ex.Message);
                CrashlyticsHelper.LogError(ex, "CierreBasicoViewModel", "ConfirmarCierre");
            }
            finally
            {
